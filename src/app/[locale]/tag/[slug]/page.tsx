...
import { getTagsWithCount, listPostsByTag } from "@/lib/db/queries";
...
export default async function TagPage({ params, searchParams }: TagPageProps) {
  ...
  const [tags, result] = await Promise.all([
    ...
    listPostsByTag(slug, locale, page),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
      ...
      {result.posts.length > 0 ? (
        ...
        <EmptyState icon={Newspaper} message={dict.tag.empty} />
      )}
      ...
      {result.totalPages > 1 ? (
        <nav className="mt-10 flex items-center justify-between">
          {result.page > 1 ? (
            <Link
              href={`/${locale}/tag/${slug}?page=${result.page - 1}`}
